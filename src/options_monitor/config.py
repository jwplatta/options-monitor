...
CANDLE_DIR: Path = Path(os.getenv("CANDLE_DIR", str(_TICKRAKE / "history" / "ibkr-paper")))
OPTIONS_DIR: Path = Path(os.getenv("OPTIONS_DIR", str(_TICKRAKE / "options" / "schwab")))
SCHWAB_CANDLE_DIR: Path = Path(
    os.getenv("SCHWAB_CANDLE_DIR", str(_TICKRAKE / "history" / "schwab"))
)
PARQUET_OPTIONS_DIR: Path = Path(os.getenv("OPTIONS_DIR", str(_TICKRAKE / "options" / "schwab")))
